...
| 2026-09-04 00:47 | 3 | 3.4 | 保存 Ctrl+S（无路径走 SaveFilePicker 钩子）/另存为 Ctrl+Shift+S（新增 AppCommands.SaveAs + 菜单项）；脏标记为 Tab 标题圆点（SavePoint/TextChanged 驱动）；关闭脏 Tab 弹 保存/不保存/取消（ConfirmUnsavedClose 钩子，MessageBox YesNoCancel）；IO 异常统一 ReportError 本地化弹窗 | 0 警告 0 错误 | 实测：WM_CHAR 注入编辑 → 圆点出现；菜单保存 → 磁盘字节更新且无 BOM 保持、圆点消失；关闭脏 Tab 提示与另存为对话框已实现，运行时验证脚本中断未完成实测（代码路径与已验证的保存/确认钩子相同） |
