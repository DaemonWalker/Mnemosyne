...
/// <summary>
/// DataWeave（.dwl）手写词法器。Lexilla 无内置 DataWeave Lexer，ScintillaHost 对此语言设 container
/// lexer，在 StyleNeeded 回调里用本类分词上色。分词规则参照官方 TextMate 语法
/// mulesoft/data-weave-tmLanguage，另补该语法未覆盖的日期字面量 |yyyy-MM-dd| 与 $/$$ 变量引用。
/// 插值表达式 $(...) 内部不做完整分词，保持字符串底色，仅高亮 $(/) 定界符与 $name。
/// </summary>
public static class DataWeaveLexer
{
    public const string LexerName = LanguageRegistry.DataWeaveLexerName;
    ...
    public readonly record struct StyleSpan(int Length, DwStyle Style);
    ...
    public static IReadOnlyList<StyleSpan> Tokenize(string text)
    {
        ...
        void Emit(int length, DwStyle style, bool? isValue = null)
        {
            ...
            {
                spans.Add(new StyleSpan(length, style));
            }
            if (isValue is { } v)
            {
                prevIsValue = v;
